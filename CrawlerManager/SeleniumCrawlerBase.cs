...
namespace CarBase.CrawlerManager
{
    public abstract class SeleniumCrawlerBase : CrawlerBase, ICrawler
    {
        ...
        protected string GetDataString(IWebElement element, string xpathFindElement, string xpathGetElement)
        {
            try
            {
                element = element.FindElement(By.XPath(xpathFindElement));
            }
            catch
            {
                return null;
        ...
        protected double GetDataFloatWithoutDot(IWebElement element, string xpathFindElement, string xpathGetElement)
        {
            ...
            if (element != null)
            {
                string s = element.FindElement(By.XPath(xpathGetElement)).Text.Replace('.', ',');
        ...
        {
            RemoteWebDriver driver = new FirefoxDriver(FirefoxDriverService.CreateDefaultService(), new FirefoxOptions(), TimeSpan.FromSeconds(120)); 
